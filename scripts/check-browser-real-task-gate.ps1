...
foreach ($path in @($sourceProvenancePath, $installedProvenancePath, $frozenProvenancePath)) {
...
foreach ($path in @($sourceProvenancePath, $installedProvenancePath, $frozenProvenancePath, $manifestPath, $PSCommandPath)) {
  ...
  if (-not $full.StartsWith($qualificationFull + '\', [System.StringComparison]::OrdinalIgnoreCase)) {
    if ($path -ne $sourceProvenancePath -and $path -ne $installedProvenancePath) {
      throw "Frozen Browser L3 gate code escaped qualification root: $path"
    }
  }
}
...
if (
  ...
  [string]$initialInstalled.playwright_mcp_version -ne '0.0.78'
) {
...
$fixtureWasLive = $null -ne $fixtureProcess -and -not $fixtureProcess.HasExited
if (-not $fixtureWasLive) { throw 'Browser L3 fixture process was not live at Finish Gate proof time.' }
