...
40. Stage 26.3B Verification Kernel/Finish Gate is accepted/closed for recorded representative scope. Stage 26.3C WorkingState/reconciliation/budget/LoopGuard is accepted/closed for the declared `verified_workspace_artifact_v1` local-Windows process-restart scope through #126; broader cross-capability recovery still requires its own acceptance. `CURRENT_STATE.md` owns immediate work and `ROADMAP.md` owns current release order; do not infer either from this constraints list.
