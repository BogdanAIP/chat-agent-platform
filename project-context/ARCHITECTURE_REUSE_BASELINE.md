...
| Automatic-review local concurrent ownership | accepted Stage 26.3C OS-backed cooperating-runner lock | exact-operation single-writer ownership before genesis/state access and through one dispatch/publication decision | persistence durability, browser-tab ownership, generic lease system | reuses a physically/review accepted process-held lock and avoids a second local concurrency framework | `AUTOMATIC_REVIEWER_RESEARCH.md`, `CURRENT_STATE.md` | `PROJECT_OWNED_REUSE_MORE` |
| Automatic-review immutable operation genesis | accepted Stage 26.3C `_exclusive_create_file` pattern | exclusive-created/fsynced exact identity + private run nonce retained for v1 so prior creation remains distinguishable from missing mutable state | mutable transition state, power-loss durability, hostile deletion detection | closes the disappearance/first-creation ambiguity using an already accepted exclusive-create primitive | `AUTOMATIC_REVIEWER_RESEARCH.md`, `CURRENT_STATE.md` | `PROJECT_OWNED_REUSE_MORE_REFINE` |
| Automatic-review mutable operation state | accepted Stage 26.3C `_write_checkpoint` / `_load_checkpoint` / identity-validation pattern | same-directory sibling temp, complete JSON candidate, `flush`, `os.fsync`, `os.replace`, strict genesis/state pair validation and fail-closed corrupt/temp-residue handling | machine/power-loss transactional guarantee, SQLite/WAL/event-log framework, browser Send ownership | separates mutable crash-restart state from live lock ownership and immutable creation evidence | `AUTOMATIC_REVIEWER_RESEARCH.md`, `CURRENT_STATE.md` | `PROJECT_OWNED_REUSE_MORE` |
| Automatic-review browser Send ownership | bounded MV3 service worker + extension-origin IndexedDB unique-key transaction | one durable same-run Send claim across tabs; only committed `add(review_run_id)` claimant may Send | local launch ownership, general browser database/runtime authority | browser tabs form a different concurrency domain from local OS processes, so a narrow transactional claim is required | `AUTOMATIC_REVIEWER_RESEARCH.md` | `PROJECT_OWNED_NARROW_NEW_ROLE` |
| Automatic-review result submission / GitHub publication | fixed `submit_independent_review_result_v1` behind `procedure_run` + project-owned endpoint-allowlisted publisher + dedicated repository-scoped GitHub App credential | reviewer gets no raw GitHub write credential/action; private run id authorizes one local submission; publisher may attempt only exact top-level PR comment POST; final full-comment-set integrity recheck remains project-owned | generic GitHub/HTTP/GraphQL proxy, reviewer-selected endpoint/method, branch/review/label/merge/settings authority, direct reviewer-held write credential | deterministic least privilege cannot be obtained from prompt instructions or GitHub permission bucket alone; keep provider credential behind local Control Plane and expose one effect | `AUTOMATIC_REVIEWER_RESEARCH.md`, `.agents/skills/code-review/SKILL.md`, `CURRENT_STATE.md`, `ROADMAP.md` | `PROJECT_OWNED_REFINE_LEAST_PRIVILEGE` |
| Automatic independent-review launch / correlation | project-owned bounded lifecycle reusing existing `procedure_run`, accepted Stage 26.3C local primitives and PR #138 deep-link/autosend mechanics | one exact-head operation, immutable private `review_run_id`, fresh-new-chat delivery and no blind relaunch after ambiguity | new scheduler/event bus, arbitrary URL/prompt launch, same-task developer wake, worker rotation | removes manual launch/paste while keeping operation identity/recovery bounded and independent from general continuation | `AUTOMATIC_REVIEWER_RESEARCH.md`, `CURRENT_STATE.md`, `ROADMAP.md` | `PROJECT_OWNED_REUSE_MORE_REFINE_EXPERIMENT` |
| Code-review evaluation harness | Harbor custom-agent/task/environment/verifier interfaces | reproducible external reviewer evaluation, frozen tasks, custom CAP adapter, metric/result collection | production launch, GitHub authority, review acceptance, Control Plane authority | avoid building a benchmark runner while keeping evaluation separate from production lifecycle | `AUTOMATIC_REVIEWER_RESEARCH.md`, `ROADMAP.md` | `SELECTED_EVALUATION_ONLY` |
...
- `AUTOMATIC_REVIEWER_RESEARCH.md` — bounded automatic-review launch/result lifecycle, accepted OS-lock + immutable genesis + crash-oriented mutable checkpoint reuse, transactionally serialized MV3/IndexedDB Send claim, deterministic one-shot result submission and endpoint-allowlisted GitHub publication, Harbor evaluation seam, benchmark-quality plan and explicit non-generalization boundaries.
