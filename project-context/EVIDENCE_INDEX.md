...
| Transport low-power manual lifecycle/tray ownership gate (PR #100) | `de0a1b32091ddfa3570c9f49631af2f1f0d4186f` | target Windows physical install and manual Start/Stop | installing the exact head while an already manager-owned runtime was running changed the tray from stale yellow to green without restarting runtime; installed manager blob matched reviewed manager; subsequent Manual Start completed in approximately one minute and reached green; Manual Stop returned to red/stopped with runtime removed. Confirms tray state is derived from persisted manager ownership rather than a stale manual receipt. |
