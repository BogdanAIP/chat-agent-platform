...
| Automatic independent-review launch / correlation / result publication | project-owned bounded lifecycle reusing existing `procedure_run`, accepted Stage 26.3C OS-backed cooperating-runner lock mechanics, PR #138 deep-link/autosend mechanics, `code-review` automatic result envelope and GitHub PR Conversation | one exact-head single-writer launch operation, durable private `review_run_id`, fresh-new-chat delivery, one structured result-evidence comment, final full-comment-set uniqueness/integrity recheck | new scheduler/event bus, new lock/lease/database framework, arbitrary URL/prompt launch, same-task developer wake, reviewer branch/approval authority | removes manual review launch/copy-paste while reusing accepted lock/consequence mechanics, keeping the six-tool surface and failing closed on concurrent launch, ambiguous delivery or late duplicate result evidence | `AUTOMATIC_REVIEWER_RESEARCH.md`, `.agents/skills/code-review/SKILL.md`, `CURRENT_STATE.md`, `ROADMAP.md` | `PROJECT_OWNED_REUSE_MORE_REFINE_EXPERIMENT` |
...
- `AUTOMATIC_REVIEWER_RESEARCH.md` — bounded automatic-review launch/result lifecycle, accepted OS-lock reuse, bounded result-publication envelope, Harbor evaluation seam, benchmark-quality plan and explicit non-generalization boundaries.
