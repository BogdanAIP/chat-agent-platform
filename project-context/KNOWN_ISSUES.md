...
5. **Первый versioned release/tag ещё не выпускался.** Release packaging должен пройти CI до merge; первый `vX.Y.Z` остаётся отдельной осознанной операцией. Пока private repo не поддерживает бесплатную GitHub artifact attestation на текущем доступе.
