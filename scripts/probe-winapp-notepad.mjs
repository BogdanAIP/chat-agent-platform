#!/usr/bin/env node
// Qualification-only inspection of one caller-specified Notepad window.
// No input, invoke, set-value, screenshot, save, or CAP public route.
import { spawnSync } from 'node:child_process';

function parseArgs(argv) {
  if (argv.length !== 4 || argv[0] !== '--pid' || argv[2] !== '--hwnd'
      || !/^[1-9][0-9]*$/.test(argv[1]) || !/^[1-9][0-9]*$/.test(argv[3])) {
    throw new Error('Usage: node scripts/probe-winapp-notepad.mjs --pid <Notepad PID> --hwnd <Notepad HWND>');
  }
...
function countControls(nodes, counts = {}) {
...
try {
  ...
  const { pid, hwnd } = parseArgs(process.argv.slice(2));
  const status = readWinAppJson(['status', '-w', String(hwnd)]);
  if (status.processId !== pid || status.hwnd !== hwnd
      || !/^notepad(?:\.exe)?$/i.test(status.processName ?? '')) {
    throw new Error('Specified window does not belong to the expected Notepad process; no inspect performed');
  }
  ...
  console.log(JSON.stringify({
    result: 'OBSERVED_ONLY', pid, hwnd,
    editor_control_types: countControls(tree.windows[0].elements),
