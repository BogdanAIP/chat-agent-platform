...
| Transport low-power Manual + OFF idle gate (PR #100) | `de0a1b32091ddfa3570c9f49631af2f1f0d4186f` | target Windows interactive physical observation; PR #100 conversation evidence | final installed head in `manual / stopped`; 60 s idle sample: `UniqueOtherPwshPidsSeen=1` (status indicator only), `OtherPwshCpuSeconds=0`, `TunnelClientCount=0`, `NodeCount=0`, `ManagerOwnerExists=False`; surviving tray PID `19080` accumulated `0.00` CPU seconds. Proves zero recurring platform PowerShell/runtime work in Manual + OFF for the observed interval. |
| Transport low-power Automatic 30-minute dormant gate (PR #100) | `de0a1b32091ddfa3570c9f49631af2f1f0d4186f` | target Windows interactive physical observation; first automatic snapshot `08/24/2026 21:12:48` | switching Manual -> Automatic was detected after `6.8 s`; one immediate Reconcile wrote the new snapshot; after another 60 s `ObservedAfter60Seconds` was unchanged and `SnapshotStayedDormant=True`, with `RuntimeReady=True`, `MCPReady=True`, `TunnelReady=True`. Health was truthfully `REMOTE_METADATA_UNAVAILABLE`, a non-destructive `wait_and_probe` class; no 10-second reconcile/status cadence returned. |
| Transport low-power manual lifecycle/tray ownership gate (PR #100) | `de0a1b32091ddfa3570c9f49631af2f1f0d4186f` | target Windows physical install and manual Start/Stop | installing the exact head while an already manager-owned runtime was running changed the tray from stale yellow to green without restarting runtime; installed manager blob matched reviewed manager; subsequent Manual Start completed in approximately one minute and reached green; Manual Stop returned to red/stopped with runtime removed. This historical gate proved ownership/lifecycle handling; its ownership-only green interpretation is superseded by the stricter `092081be7d99dbeee6f092a6d48066d1a95e37c2` readiness gate below. |
| Transport low-power final truthful-green + ordinary-Chat ON/OFF gate (PR #100) | `092081be7d99dbeee6f092a6d48066d1a95e37c2` | target Windows exact install; PR #100 comments `5406710415` (ON) and `5406744178` (OFF); ordinary ChatGPT / `Chat Local Bridge Test`; workspace `C:\Users\eahra\AppData\Local\ChatAgentPlatform\stage26\ordinary-chat-E4F49B4A` | all seven required GitHub workflows passed on the exact head. Manual Start reached green only after current-run local readiness plus control-plane poll confirmation. With Manual ON/green, fresh `workspace_read(operation=roots)` succeeded and returned the exact allowed workspace. After explicit Manual OFF/red, the same ordinary-Chat call failed before local execution with `Transport send error ... HTTP 504`, proving the ChatGPT route was unavailable when the local runtime was stopped. Repeated successful ON calls were observed at approximately `51 s -> 16 s -> 5.085 s`; this is diagnostic latency only, not a correctness failure, and performance tuning is explicitly deferred. |
| Stage 26.3A normal six-tool ordinary-Chat verified procedure runtime | `300db9956dfbdf0300ecc59f017d6f3280d4353a` | target Windows ordinary ChatGPT / `Chat Local Bridge Test`; workspace `C:\Users\eahra\AppData\Local\ChatAgentPlatform\stage26\ordinary-chat-E4F49B4A`; challenge `STAGE26_3A_CHAT_E2E_E4F49B4AD4CB4DABA07A9F01A5575255`; completion task `497ecb591779219ef0ee1e55ea7ad0b8`; zero-overwrite task `02b09a4909b6d71e0578c19b2d395cb8` | normal `semantic` + `direct-stdio`, `1MCP_REQUIRED=False`; all six semantic tools used in one long-horizon autonomous research task; 16 content pages / 12 works or benchmark groups / 12 successful browser transitions; `research-ledger.md` used and reread as working memory; `gui-agent-research.md` written and independently reread; `procedure_run` completed exactly 3 verified actions and produced `.chat-agent-platform/stage26-3a/ordinary-chat-result.txt` with SHA-256 `2396b8338edced2675982db9d263a046705f7f906b553b0ed19b81f51205e583`; independent `workspace_read` matched the expected success nonce; second `procedure_run` returned `status=abstained`, `action_count=0`, `escalation_reason=target_already_exists`; independent reread proved identical content/SHA and zero overwrite. One invalid `web_interact` call was recovered by re-observing and supplying the explicit target. |
...
Transport-call latency optimization is intentionally **not** a current acceptance blocker. The final PR #100 gate established correctness and route availability semantics; cold/warm latency profiling may be revisited later if it materially affects product use.
