...
| Automatic-review deep-link/composer mechanics | PR #138 experiment + ChatGPT Temporary Chat launch intent | run-bound fresh Temporary-Chat delivery/composer mechanics plus live non-personalized-state proof before Send/result extraction | URL-only qualification, page-local ownership, general scheduler authority | retain proved fresh-chat UI mechanics while refining them around the plugin-free Temporary Chat security context | `AUTOMATIC_REVIEWER_AUTHORITY_RESEARCH.md`, `AUTOMATIC_REVIEWER_RESEARCH.md` | `REFINE` |
...
| Automatic-review reviewer authority qualification | non-personalized ordinary ChatGPT Temporary Chat on personal Plus; managed read-only Action Control remains a deployment alternative | remove the entire plugin action surface from the semantic reviewer and positively revalidate Temporary/non-personalized page state before Send and result extraction | per-message non-selection, approval prompts, broad reviewer plugin access, relying only on URL intent | the 2026-08-27 Temporary Chat behavior provides a personal-plan isolation primitive where non-personalized chats expose no plugins, avoiding a second-account requirement while preserving actual GitHub mutation-action unreachability | `AUTOMATIC_REVIEWER_AUTHORITY_RESEARCH.md`, `AUTOMATIC_REVIEWER_RESEARCH.md`, `.agents/skills/code-review/SKILL.md` | `SELECTED_REVALIDATE_PHYSICAL` |
| Automatic-review local result submission/reconciliation | accepted fixed local state machine + submit-only reviewer Native Messaging host | deterministically transport one exact completed review result into the same nonce/identity/result-validation state machine and preserve same-lock manual closure | generic Native Messaging RPC, six-tool bridge access, caller-supplied state root/procedure/path/command/URL, GitHub writes | Temporary Chat intentionally removes plugins including Local Bridge; a one-action native host preserves least privilege without exposing broader MCP authority | `AUTOMATIC_REVIEWER_AUTHORITY_RESEARCH.md`, `AUTOMATIC_REVIEWER_RESEARCH.md`, `.agents/skills/code-review/SKILL.md`, `CURRENT_STATE.md` | `REFINE` |
| Automatic independent-review launch / correlation | `procedure_run` + accepted local primitives + refined PR #138/Temporary Chat mechanics | one exact-head operation, private run id, fresh Temporary-Chat delivery and no blind relaunch | new scheduler/event bus, arbitrary launcher, same-task developer wake, worker rotation | remove routine launch/paste while keeping operation identity/recovery bounded and separating review semantics from deterministic browser/native transport | `AUTOMATIC_REVIEWER_AUTHORITY_RESEARCH.md`, `AUTOMATIC_REVIEWER_RESEARCH.md`, `CURRENT_STATE.md`, `ROADMAP.md` | `REFINE` |
...
For a role absent from this baseline at the start of the stage, record it as `NEW_ARCHITECTURE` in the Brief and run the Research Scope Expansion Gate. Scope labels such as `NARROW`, `EVALUATION_ONLY` or `MANUAL` belong in the reason/scope text, not as second lineage decisions.
...
- `AUTOMATIC_REVIEWER_RESEARCH.md` — bounded automatic-review exact-operation/local-result lifecycle, filesystem/IndexedDB solution evidence and Harbor evaluation seam.
- `AUTOMATIC_REVIEWER_AUTHORITY_RESEARCH.md` — focused authority/handoff re-entry for non-personalized Temporary Chat and the submit-only Native Messaging transport selected for the personal-Plus path.
