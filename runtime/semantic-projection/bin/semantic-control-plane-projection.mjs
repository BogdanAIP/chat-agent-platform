...
function controlPlaneEnvironment(request, assignedTaskId = null) {
  ...
  // New workspace-artifact correlation is generated by this reviewed parent and
  // is deliberately not inherited through SAFE_CHILD_ENV_ALLOWLIST. The public
  // caller cannot choose this value; it is a child-only correlation channel.
  if (assignedTaskId !== null) {
    env.CHAT_PROCEDURE_ASSIGNED_TASK_ID = assignedTaskId;
  }
  return env;
}
...
function runProcedure(request) {
  return new Promise((resolve) => {
    ...
    let python;
    try {
      env = controlPlaneEnvironment(request, assignedTaskId);
...
const workspaceArtifactProcedureSchema = z.object({
  procedure: z.literal(WORKSPACE_ARTIFACT_PROCEDURE),
