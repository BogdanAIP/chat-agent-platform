...
| Agent-session first-provider browser delivery ownership | project `chatgpt-temporary` MV3 service worker + ephemeral live owner record + extension-origin IndexedDB unique-key claim | one neutral-preflight owner per delegation/delivery in one MV3 lifetime, same-owner ambiguous-commit reconciliation and prepared-state handle/preflight rebind, one task self-navigation, durable same-delivery claim across tabs, exact correlation and no second Send | durable browser/session lease, provider conversation identity, PowerShell task-navigation authority, local generic Delegation-state ownership, generic browser database/runtime authority | separate ephemeral browser-lifetime launch ownership from durable one-Send delivery ownership; controller ACK/restart ambiguity can reconcile only while the original MV3 owner survives, while browser/MV3 loss remains fail closed | `AGENT_SESSION_TEMPORARY_PREFLIGHT_OWNER_REBIND_REENTRY.md`, `AGENT_SESSION_TEMPORARY_PREFLIGHT_COMMIT_REENTRY.md`, `AGENT_SESSION_DELEGATION_REENTRY.md`, `CURRENT_STATE.md` | `REFINE` |
...
| Native systems execution host | project-owned Rust Native Host v1 after Core freeze | narrow Windows process/process-tree ownership, Job Objects/native handles, bounded launch/cancel/reap receipts; later ConPTY mechanics through the same host boundary | planning, capability grants, WorkingState/reconciliation, Verification Kernel, Finish Gate, generic Chat-facing shell/local-code authority, provider discovery | the frozen Core needs one replaceable native systems executor below authority; current evidence supports a narrow Rust boundary while the historical Rust-first Control Plane is explicitly not revived | `ARCHITECTURE.md`, `ROADMAP.md`, `NATIVE_HOST_V1_RUST_REENTRY.md` | `SELECTED_NARROW` |
| Capability-spanning operational state | project-owned `WorkingState` | constraints, subgoals/progress, provenance/freshness, evidence refs, recovery/reconciliation history, budgets, grants/procedure/delegation refs; active-grant replacement is revisioned, evidence-bound and blocked while mutation outcome is unresolved | OpenAdapt procedure state, provider/session task stores, generic delegation state as replacement, private chain-of-thought | no external procedure/session runtime or narrow delegation lifecycle spans the full product authority/state boundary; forward authority changes must not rewrite historical attempt authority | `CONTROL_PLANE.md`, `ARCHITECTURE.md`, `ROADMAP.md`, `DECISIONS.md`, `WORKSPACE_LEGACY_GRANT_HANDOFF_REENTRY.md` | `PROJECT_OWNED` |
...
- `AGENT_SESSION_TEMPORARY_PREFLIGHT_COMMIT_REENTRY.md` / `AGENT_SESSION_TEMPORARY_PREFLIGHT_OWNER_REBIND_REENTRY.md` — current first-provider browser launch-owner reconciliation refinement; no persistent-session authority.
