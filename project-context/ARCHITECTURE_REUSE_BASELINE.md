...
| Automatic independent-review launch / correlation / result publication | project-owned bounded lifecycle reusing PR #138 deep-link/autosend mechanics + existing `procedure_run` + GitHub PR conversation | one exact-head launch operation, fresh-new-chat delivery, bounded `review_run_id`, one structured result-evidence comment | generic scheduler/event bus, arbitrary URL/prompt launch, same-task developer wake, reviewer branch/approval authority | removes manual review launch/copy-paste while keeping the existing six-tool surface and current review authority model | `AUTOMATIC_REVIEWER_RESEARCH.md`, `CURRENT_STATE.md`, `ROADMAP.md` | `PROJECT_OWNED_REFINE_EXPERIMENT` |
...
- `AUTOMATIC_REVIEWER_RESEARCH.md` — bounded automatic-review launch/result lifecycle, Harbor evaluation seam, benchmark-quality plan and explicit non-generalization boundaries.
