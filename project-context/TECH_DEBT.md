...
| TD-008 | **P3** | Two old maintenance branches still contain graph-unique work | They cannot be deleted purely as stale refs, but their snapshots are now far behind current architecture and contain superseded assertions. | Immediate post-26.3B hygiene sweep | Review/port only still-valid ideas through fresh PRs, explicitly mark superseded content, then delete both branches. |
...
Current post-#119 comparison against live main:
...
`maintenance/tech-debt-architecture-ci` cannot be merged as-is: its useful dynamic PowerShell-discovery idea is already present in current CI, while its old acceptance assertions are stale.

`maintenance/slim-live-context-docs` also cannot be merged as-is: its snapshot predates the six-tool accepted runtime and later Stage 26 work. Its useful principle—keep live context slim and send exact evidence to `EVIDENCE_INDEX.md`—has been ported into current documentation through the post-26.3B live-context sync.

After the sync is merged and reviewed, recompare both branches. If no still-valid unique behavior remains, delete them as superseded rather than carrying them indefinitely.
...
TD-010 before/with first 26.3C Browser/recovery runtime touch
 ...
 -> resolve TD-008 after current live-context sync
