...
| Automatic independent-review launch / correlation / result publication | project-owned bounded lifecycle reusing existing `procedure_run`, accepted Stage 26.3C OS-backed cooperating-runner lock mechanics, PR #138 deep-link/autosend mechanics, a bounded MV3 service-worker + extension-origin IndexedDB Send claim, `code-review` automatic result envelope and GitHub PR Conversation | one exact-head single-writer local launch operation, durable private `review_run_id`, fresh-new-chat delivery, exactly one browser-side Send claim per run, one structured result-evidence comment, final full-comment-set uniqueness/integrity recheck | new scheduler/event bus, new local lock/lease/database framework, non-transactional content-script Send ownership, general browser database/runtime authority, arbitrary URL/prompt launch, same-task developer wake, reviewer branch/approval authority | removes manual review launch/copy-paste while reusing accepted local lock/consequence mechanics and adding a narrowly scoped transactionally serialized browser Send claim; keeps the six-tool surface and fails closed on concurrent launch, concurrent tabs, ambiguous delivery or late duplicate result evidence | `AUTOMATIC_REVIEWER_RESEARCH.md`, `.agents/skills/code-review/SKILL.md`, `CURRENT_STATE.md`, `ROADMAP.md` | `PROJECT_OWNED_REUSE_MORE_REFINE_EXPERIMENT` |
...
- `AUTOMATIC_REVIEWER_RESEARCH.md` — bounded automatic-review launch/result lifecycle, accepted OS-lock reuse, transactionally serialized MV3/IndexedDB browser Send claim, bounded result-publication envelope, Harbor evaluation seam, benchmark-quality plan and explicit non-generalization boundaries.
