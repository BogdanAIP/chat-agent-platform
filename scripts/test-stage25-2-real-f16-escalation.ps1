...
$Harness = Join-Path $SemanticRoot 'tests\target-stage25-2-real-f16-escalation.mjs'
...
if ([string]$env:CHAT_VISION_RUNTIME_TEST_MODE -eq '1') {
    throw 'Real Stage 25.2 F16 acceptance refuses CHAT_VISION_RUNTIME_TEST_MODE=1.'
}
...
foreach ($required in @($Harness, $RuntimeController, $RuntimeHelper, $Python)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) {
        throw "Required Stage 25.2 target-acceptance file is missing: $required"
    }
}
...
if (-not $chromeRunningBefore) {
    throw 'Stage 25.2 target acceptance requires the user Chrome workload to remain open.'
}

Write-Host "`n===== STAGE 25.2 PUBLIC SEMANTIC -> REAL F16 ACCEPTANCE =====" -ForegroundColor Cyan
...
if ($LASTEXITCODE -ne 0) { throw "Vision runtime Doctor failed:`n$doctorText" }
...
if ([bool]$statusBefore.running) {
    Write-Host 'Stopping the currently owned vision runtime before semantic-first cold-start acceptance.' -ForegroundColor Yellow
    $null = (& $RuntimeController -Action Stop | Out-String)
}

if ([string]::IsNullOrWhiteSpace($ResultPath)) {
    $stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
    $resultRoot = Join-Path $env:LOCALAPPDATA "ChatAgentPlatform\stage25\runtime\stage25-2-public-escalation-$stamp"
...
$psi.Environment['STAGE25_2_RESULT_PATH'] = $ResultPath
...
try {
    Write-Host "`n===== LIVE PUBLIC ESCALATION HARNESS =====" -ForegroundColor Cyan
    $clientProc = [System.Diagnostics.Process]::Start($psi)
    if ($null -eq $clientProc) { throw 'Could not start Stage 25.2 Node target harness.' }

    while (-not $clientProc.HasExited) {
        Start-Sleep -Milliseconds 500
        $os = Get-CimInstance Win32_OperatingSystem
        $freeGB = ($os.FreePhysicalMemory * 1KB) / 1GB
        if ($freeGB -lt $minRamGB) { $minRamGB = $freeGB }
    ...
    if (Test-Path -LiteralPath $ResultPath -PathType Leaf) {
        ...
        throw "Stage 25.2 target harness did not create result file: $ResultPath"
    }

    if ($safetyStop) {
        throw "Stage 25.2 target acceptance stopped because free RAM fell below $EmergencyRamFloorGB GB."
    }
    if ($clientProc.ExitCode -ne 0) {
        throw "Stage 25.2 public semantic -> F16 target harness failed with exit code $($clientProc.ExitCode)."
    }
}
finally {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { Write-Warning "Vision runtime Stop failed: $stopText" }
    }
    catch { Write-Warning "Vision runtime Stop failed: $($_.Exception.Message)" }
    ...
    catch { Write-Warning "Could not read final vision runtime status: $($_.Exception.Message)" }
