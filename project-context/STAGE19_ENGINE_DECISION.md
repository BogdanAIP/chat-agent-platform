...
- the selected engine is Matchering 2.0.6, invoked through a fixed Python adapter process;
- the dedicated engine environment is pinned to Python 3.10 because Matchering 2.0.6 upstream explicitly classifies Python 3.8, 3.9 and 3.10; the core Python runtime does not become a Matchering dependency;
- the adapter itself technically rejects any installed Matchering version other than 2.0.6;
...
- final output sample rate is explicitly restored to and validated against the original TARGET rate rather than accepting Matchering/FFmpeg implicit resampling;
...
Acceptance evidence is recorded in `project-context/STAGE19_REFERENCE_MASTERING.md`: core Windows CI #159 and the real pinned Matchering E2E #36 both passed on code head `1cb74fe5771bf9e143a9cdecdbc632e4eeb15ec2`.
