...
| Automatic-review local concurrent ownership | accepted Stage 26.3C OS-backed cooperating-runner lock | exact-operation single-writer ownership before record access/nonce generation and through the one dispatch decision | persistence durability, browser-tab ownership, generic lease system | reuses a physically/review accepted process-held lock and avoids a second local concurrency framework | `AUTOMATIC_REVIEWER_RESEARCH.md`, `CURRENT_STATE.md` | `PROJECT_OWNED_REUSE_MORE` |
| Automatic-review durable local operation record | accepted Stage 26.3C `_write_checkpoint` / `_load_checkpoint` / identity-validation pattern | same-directory sibling temp, complete JSON candidate, `flush`, `os.fsync`, `os.replace`, strict schema/exact-identity load and fail-closed corrupt/temp-residue handling | machine/power-loss transactional guarantee, SQLite/WAL/event-log framework, browser Send ownership | separates crash-atomic process-restart persistence from live lock ownership and reuses an already accepted project persistence role | `AUTOMATIC_REVIEWER_RESEARCH.md`, `CURRENT_STATE.md` | `PROJECT_OWNED_REUSE_MORE` |
| Automatic independent-review launch / correlation / result publication | project-owned bounded lifecycle reusing existing `procedure_run`, the two accepted Stage 26.3C local primitives, PR #138 deep-link/autosend mechanics, a bounded MV3 service-worker + extension-origin IndexedDB Send claim, `code-review` automatic result envelope and GitHub PR Conversation | one exact-head launch operation, durable private `review_run_id`, fresh-new-chat delivery, exactly one browser-side Send claim per run, one structured result-evidence comment, final full-comment-set uniqueness/integrity recheck | new scheduler/event bus, new local lock/persistence framework, non-transactional content-script Send ownership, general browser database/runtime authority, arbitrary URL/prompt launch, same-task developer wake, reviewer branch/approval authority | removes manual review launch/copy-paste while keeping concurrency, crash persistence, browser Send ownership and result evidence as separate fail-closed boundaries | `AUTOMATIC_REVIEWER_RESEARCH.md`, `.agents/skills/code-review/SKILL.md`, `CURRENT_STATE.md`, `ROADMAP.md` | `PROJECT_OWNED_REUSE_MORE_REFINE_EXPERIMENT` |
...
- `AUTOMATIC_REVIEWER_RESEARCH.md` — bounded automatic-review launch/result lifecycle, separate accepted OS-lock and crash-atomic checkpoint reuse, transactionally serialized MV3/IndexedDB browser Send claim, bounded result-publication envelope, Harbor evaluation seam, benchmark-quality plan and explicit non-generalization boundaries.
