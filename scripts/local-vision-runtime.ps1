...
function Get-Config {
    ...
    try {
        $config = Get-Content -LiteralPath $EffectiveConfigPath -Raw -Encoding utf8 | ConvertFrom-Json
    }
    catch {
    ...
    $hostName = [string]$config.runtime.host
    if ($hostName -ne '127.0.0.1') { throw 'Vision runtime must bind only to 127.0.0.1.' }
    ...
    if ($watchdogInterval -lt 1 -or $watchdogInterval -gt 60) { throw 'watchdog_interval_seconds must be between 1 and 60.' }
    if ($watchdogInterval -gt $ttl) { throw 'watchdog_interval_seconds must not exceed idle_ttl_seconds.' }
...
function Get-ArtifactPaths {
    ...
    foreach ($value in @($directory, $modelFile, $mmprojFile)) {
        if ([string]::IsNullOrWhiteSpace($value)) { throw 'Artifact directory/file names must be non-empty.' }
        if ($value -match '[\\/]\.\.?([\\/]|$)' -or [System.IO.Path]::IsPathRooted($value)) {
            throw 'Artifact paths in the reviewed config must remain relative.'
        }
    }

    $artifactDir = Join-Path $EffectiveModelRoot $directory
    return [pscustomobject]@{
        model = Join-Path $artifactDir $modelFile
        mmproj = Join-Path $artifactDir $mmprojFile
    }
}
...
function Confirm-Artifacts {
    ...
    $mmprojSha = Test-Artifact `
        ...
        -Label 'mmproj'
    return [pscustomobject]@{
        model_path = [System.IO.Path]::GetFullPath($paths.model)
        mmproj_path = [System.IO.Path]::GetFullPath($paths.mmproj)
...
function Confirm-RuntimeVersion {
    ...
    foreach ($marker in @($Config.runtime.required_version_markers | ForEach-Object { [string]$_ })) {
        if ([string]::IsNullOrWhiteSpace($marker) -or $output -notmatch [regex]::Escape($marker)) {
...
function Test-OwnedServerProcess {
    ...
    if ([string]::IsNullOrWhiteSpace($actualExe)) { return $false }
    try { $actualExe = [System.IO.Path]::GetFullPath($actualExe) } catch { return $false }
    if ($actualExe -ine [System.IO.Path]::GetFullPath([string]$State.runtime_executable)) { return $false }

    $commandLine = [string]$process.CommandLine
    foreach ($marker in @(
        [string]$State.model_path,
        [string]$State.mmproj_path,
        '--host', [string]$State.host,
        '--port', [string]$State.port
    )) {
        if (-not (Contains-CommandMarker -CommandLine $commandLine -Marker $marker)) { return $false }
    }

    try {
        $running = Get-Process -Id $processId -ErrorAction Stop
        $actualStart = $running.StartTime.ToUniversalTime().ToString('o')
...
function Test-Health {
    param([Parameter(Mandatory)][int]$Port)
    try {
        $response = Invoke-WebRequest -Uri "http://127.0.0.1:$Port/health" -TimeoutSec 2 -SkipHttpErrorCheck
...
function Get-StatusObject {
    ...
    $processExists = $false
    try { $processExists = ($null -ne (Get-Process -Id ([int]$state.pid) -ErrorAction SilentlyContinue)) } catch {}
    ...
    try {
        ...
        $idleSeconds = [math]::Max(0, [math]::Round(([DateTimeOffset]::UtcNow - $lastUsed).TotalSeconds, 1))
    }
    catch {}
    ...
    return [pscustomobject]@{
        ...
        state = if ($conflict) { 'ownership-conflict' } elseif ($ready) { 'ready' } elseif ($owned) { 'starting-or-unhealthy' } else { 'stale' }
    }
}
...
function Start-Runtime {
    ...
    if ($null -ne $existing) {
        if (Test-OwnedServerProcess -State $existing) {
            ...
            $processExists = ($null -ne (Get-Process -Id ([int]$existing.pid) -ErrorAction SilentlyContinue))
            if ($processExists) {
    ...
    $listeners = @(Get-PortListeners -Port ([int]$Config.runtime.port))
    ...
    $runtimeExecutable = Resolve-RuntimeExecutable -Config $Config
    $version = Confirm-RuntimeVersion -Config $Config -Executable $runtimeExecutable

    $arguments = [System.Collections.Generic.List[string]]::new()
    foreach ($value in @(Get-CommandPrefix -Config $Config)) { $arguments.Add([string]$value) }
    foreach ($value in @('-m', $artifacts.model_path, '--mmproj', $artifacts.mmproj_path)) { $arguments.Add([string]$value) }
    foreach ($value in @($Config.server_args | ForEach-Object { [string]$_ })) { $arguments.Add([string]$value) }
    foreach ($value in @('--host', '127.0.0.1', '--port', [string][int]$Config.runtime.port)) { $arguments.Add([string]$value) }
    ...
    $state = [pscustomobject][ordered]@{
        ...
        profile = [string]$Config.profile
        pid = [int]$process.Id
        process_start_time_utc = $processStart
    ...
    $ready = $false
    $deadline = [DateTimeOffset]::UtcNow.AddSeconds([int]$Config.runtime.ready_timeout_seconds)
    while ([DateTimeOffset]::UtcNow -lt $deadline) {
        if ($process.HasExited) { break }
        if (Test-Health -Port ([int]$Config.runtime.port)) { $ready = $true; break }
        Start-Sleep -Milliseconds 250
    }
    if (-not $ready) {
        $stderrTail = if (Test-Path -LiteralPath $StderrLog) { (Get-Content -LiteralPath $StderrLog -Tail 60 | Out-String).Trim() } else { '' }
        $null = Stop-OwnedRuntime -Config $Config -Reason 'readiness-failed'
        throw "Vision runtime did not become ready. $stderrTail"
    }

    $postMemory = Get-MemorySnapshot
    if (-not (Test-MemoryFloor `
        -Memory $postMemory `
        ...
        throw "Vision runtime started but violated run memory floor: physical=$($postMemory.physical_gb) GB, virtual=$($postMemory.virtual_gb) GB."
    }

    $watchdog = Start-Watchdog -Config $Config -State $state
    if ($null -ne $watchdog) {
        $state = Get-State
...
$mutexName = if ($TestMode) {
    $suffixBytes = [Text.Encoding]::UTF8.GetBytes($EffectiveStateRoot)
    $sha = [Security.Cryptography.SHA256]::Create()
    try { 'Local\ChatAgentPlatformVisionRuntime-' + ([Convert]::ToHexString($sha.ComputeHash($suffixBytes))).Substring(0, 12) }
    finally { $sha.Dispose() }
}
else {
